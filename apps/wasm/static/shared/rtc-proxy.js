// WebRTC Proxy - Runs on main thread, proxies RTCPeerConnection calls from worker
//
// The WASM module runs in a Web Worker, but RTCPeerConnection is only available
// on the main thread. This proxy receives requests from the worker and manages
// the actual WebRTC connections.

/**
 * RTCProxy - Manages WebRTC connections on behalf of the worker
 */
export class RTCProxy {
    constructor() {
        // Map of registryId -> { pc: RTCPeerConnection }
        this._peerConnections = new Map();

        // Map of dcHandle -> { channel: RTCDataChannel, registryId: number }
        this._dataChannels = new Map();
        this._nextDcHandle = 1;

        // Callback to send messages back to the worker
        this._sendToWorker = null;
    }

    /**
     * Set the callback for sending messages to the worker
     * @param {Function} callback - Function that takes (type, payload)
     */
    setSendCallback(callback) {
        this._sendToWorker = callback;
    }

    /**
     * Handle a message from the worker
     * @param {string} type - Message type (e.g., 'rtc_create')
     * @param {object} payload - Message payload
     */
    handleMessage(type, payload) {
        // Only log non-frequent messages (skip high-frequency dc_send/dc_on_message for presence)
        if (type !== 'dc_send' && type !== 'dc_on_message') {
            console.log('[RTCProxy]', type, payload);
        }
        switch (type) {
            case 'rtc_create':
                return this._createPeerConnection(payload);
            case 'rtc_destroy':
                return this._destroyPeerConnection(payload);
            case 'rtc_close':
                return this._closePeerConnection(payload);
            case 'rtc_create_offer':
                return this._createOffer(payload);
            case 'rtc_create_answer':
                return this._createAnswer(payload);
            case 'rtc_set_local_description':
                return this._setLocalDescription(payload);
            case 'rtc_set_remote_description':
                return this._setRemoteDescription(payload);
            case 'rtc_add_ice_candidate':
                return this._addIceCandidate(payload);
            case 'rtc_create_data_channel':
                return this._createDataChannel(payload);
            // DataChannel operations
            case 'dc_send':
                return this._dcSend(payload);
            case 'dc_close':
                return this._dcClose(payload);
            case 'dc_set_buffered_amount_low_threshold':
                return this._dcSetBufferedAmountLowThreshold(payload);
            default:
                console.warn('RTCProxy: Unknown message type:', type);
                return { error: 'Unknown message type' };
        }
    }

    // ========================================================================
    // PeerConnection operations
    // ========================================================================

    _createPeerConnection({ iceServers, registryId }) {
        const config = {};
        if (iceServers) {
            config.iceServers = iceServers;
        }

        try {
            const pc = new RTCPeerConnection(config);
            this._peerConnections.set(registryId, { pc });

            // Setup event handlers
            this._setupPeerConnectionCallbacks(registryId, pc);

            return {};
        } catch (err) {
            console.error('RTCProxy: Failed to create PeerConnection:', err);
            return { error: err.message };
        }
    }

    _setupPeerConnectionCallbacks(registryId, pc) {
        pc.onconnectionstatechange = () => {
            this._send('rtc_on_connection_state', {
                registryId,
                state: this._mapConnectionState(pc.connectionState)
            });
        };

        pc.oniceconnectionstatechange = () => {
            this._send('rtc_on_ice_connection_state', {
                registryId,
                state: this._mapIceConnectionState(pc.iceConnectionState)
            });
        };

        pc.onicegatheringstatechange = () => {
            this._send('rtc_on_ice_gathering_state', {
                registryId,
                state: this._mapIceGatheringState(pc.iceGatheringState)
            });
        };

        pc.onsignalingstatechange = () => {
            this._send('rtc_on_signaling_state', {
                registryId,
                state: this._mapSignalingState(pc.signalingState)
            });
        };

        pc.onicecandidate = (event) => {
            if (event.candidate) {
                this._send('rtc_on_ice_candidate', {
                    registryId,
                    candidate: event.candidate.candidate,
                    sdpMid: event.candidate.sdpMid,
                    sdpMLineIndex: event.candidate.sdpMLineIndex
                });
            } else {
                // End of candidates
                this._send('rtc_on_ice_candidate', {
                    registryId,
                    candidate: null,
                    sdpMid: null,
                    sdpMLineIndex: 0
                });
            }
        };

        pc.ondatachannel = (event) => {
            const channel = event.channel;
            const dcHandle = this._nextDcHandle++;
            this._dataChannels.set(dcHandle, { channel, registryId });
            this._setupDataChannelCallbacks(dcHandle, channel, registryId);

            this._send('rtc_on_data_channel', {
                registryId,
                dcHandle,
                label: channel.label
            });
        };

        pc.onnegotiationneeded = () => {
            this._send('rtc_on_negotiation_needed', { registryId });
        };
    }

    _destroyPeerConnection({ registryId }) {
        const entry = this._peerConnections.get(registryId);
        if (entry) {
            entry.pc.close();
            this._peerConnections.delete(registryId);
        }
        return {};
    }

    _closePeerConnection({ registryId }) {
        const entry = this._peerConnections.get(registryId);
        if (entry) {
            entry.pc.close();
        }
        return {};
    }

    async _createOffer({ registryId }) {
        const entry = this._peerConnections.get(registryId);
        if (!entry) {
            this._send('rtc_on_create_sdp', {
                registryId,
                success: false,
                error: 'PeerConnection not found'
            });
            return;
        }

        try {
            const offer = await entry.pc.createOffer();
            this._send('rtc_on_create_sdp', {
                registryId,
                success: true,
                sdp: offer.sdp,
                type: 'offer'
            });
        } catch (err) {
            this._send('rtc_on_create_sdp', {
                registryId,
                success: false,
                error: err.message
            });
        }
    }

    async _createAnswer({ registryId }) {
        const entry = this._peerConnections.get(registryId);
        if (!entry) {
            this._send('rtc_on_create_sdp', {
                registryId,
                success: false,
                error: 'PeerConnection not found'
            });
            return;
        }

        try {
            const answer = await entry.pc.createAnswer();
            this._send('rtc_on_create_sdp', {
                registryId,
                success: true,
                sdp: answer.sdp,
                type: 'answer'
            });
        } catch (err) {
            this._send('rtc_on_create_sdp', {
                registryId,
                success: false,
                error: err.message
            });
        }
    }

    async _setLocalDescription({ registryId, sdpType, sdp }) {
        const entry = this._peerConnections.get(registryId);
        if (!entry) {
            this._send('rtc_on_set_sdp', {
                registryId,
                success: false,
                error: 'PeerConnection not found'
            });
            return;
        }

        try {
            await entry.pc.setLocalDescription({ type: sdpType, sdp });
            this._send('rtc_on_set_sdp', {
                registryId,
                success: true
            });
        } catch (err) {
            this._send('rtc_on_set_sdp', {
                registryId,
                success: false,
                error: err.message
            });
        }
    }

    async _setRemoteDescription({ registryId, sdpType, sdp }) {
        const entry = this._peerConnections.get(registryId);
        if (!entry) {
            this._send('rtc_on_set_sdp', {
                registryId,
                success: false,
                error: 'PeerConnection not found'
            });
            return;
        }

        try {
            await entry.pc.setRemoteDescription({ type: sdpType, sdp });
            this._send('rtc_on_set_sdp', {
                registryId,
                success: true
            });
        } catch (err) {
            this._send('rtc_on_set_sdp', {
                registryId,
                success: false,
                error: err.message
            });
        }
    }

    async _addIceCandidate({ registryId, candidate, sdpMid, sdpMLineIndex }) {
        const entry = this._peerConnections.get(registryId);
        if (!entry) {
            this._send('rtc_on_set_sdp', {
                registryId,
                success: false,
                error: 'PeerConnection not found'
            });
            return;
        }

        try {
            await entry.pc.addIceCandidate({
                candidate,
                sdpMid,
                sdpMLineIndex
            });
            this._send('rtc_on_set_sdp', {
                registryId,
                success: true
            });
        } catch (err) {
            this._send('rtc_on_set_sdp', {
                registryId,
                success: false,
                error: err.message
            });
        }
    }

    _createDataChannel({ registryId, label, ordered, maxRetransmits, maxPacketLifeTime }) {
        const entry = this._peerConnections.get(registryId);
        if (!entry) {
            console.error('RTCProxy: PeerConnection not found for createDataChannel');
            return { error: 'PeerConnection not found' };
        }

        const options = { ordered };
        if (maxRetransmits >= 0) options.maxRetransmits = maxRetransmits;
        if (maxPacketLifeTime >= 0) options.maxPacketLifeTime = maxPacketLifeTime;

        try {
            const channel = entry.pc.createDataChannel(label, options);
            const dcHandle = this._nextDcHandle++;
            this._dataChannels.set(dcHandle, { channel, registryId });
            this._setupDataChannelCallbacks(dcHandle, channel, registryId);

            // Notify worker about the created channel via callback
            // (same path as ondatachannel for consistency)
            this._send('rtc_on_data_channel', {
                registryId,
                dcHandle,
                label
            });

            return {};
        } catch (err) {
            console.error('RTCProxy: Failed to create DataChannel:', err);
            return { error: err.message };
        }
    }

    // ========================================================================
    // DataChannel operations
    // ========================================================================

    _setupDataChannelCallbacks(dcHandle, channel, registryId) {
        channel.onopen = () => {
            this._send('dc_on_open', { dcHandle, registryId });
        };

        channel.onclose = () => {
            this._send('dc_on_close', { dcHandle, registryId });
        };

        channel.onerror = (event) => {
            this._send('dc_on_error', {
                dcHandle,
                registryId,
                error: event.error?.message || 'DataChannel error'
            });
        };

        channel.onmessage = (event) => {
            if (typeof event.data === 'string') {
                this._send('dc_on_message', {
                    dcHandle,
                    registryId,
                    isText: true,
                    data: event.data
                });
            } else if (event.data instanceof ArrayBuffer) {
                // Convert ArrayBuffer to array for transfer
                this._send('dc_on_message', {
                    dcHandle,
                    registryId,
                    isText: false,
                    data: Array.from(new Uint8Array(event.data))
                });
            }
        };
    }

    _dcSend({ dcHandle, isText, data }) {
        const entry = this._dataChannels.get(dcHandle);
        if (!entry) {
            return { error: 'DataChannel not found' };
        }

        try {
            if (isText) {
                entry.channel.send(data);
            } else {
                entry.channel.send(new Uint8Array(data).buffer);
            }
            return {};
        } catch (err) {
            return { error: err.message };
        }
    }

    _dcClose({ dcHandle }) {
        const entry = this._dataChannels.get(dcHandle);
        if (entry) {
            entry.channel.close();
            this._dataChannels.delete(dcHandle);
        }
        return {};
    }

    _dcSetBufferedAmountLowThreshold({ dcHandle, threshold }) {
        const entry = this._dataChannels.get(dcHandle);
        if (entry) {
            entry.channel.bufferedAmountLowThreshold = threshold;
        }
        return {};
    }

    // ========================================================================
    // Helper methods
    // ========================================================================

    _send(type, payload) {
        // Only log non-frequent messages (skip high-frequency dc_on_message for presence)
        if (type !== 'dc_on_message') {
            console.log('[RTCProxy] -> worker:', type, payload);
        }
        if (this._sendToWorker) {
            this._sendToWorker(type, payload);
        }
    }

    _mapConnectionState(state) {
        switch (state) {
            case 'new': return 0;
            case 'connecting': return 1;
            case 'connected': return 2;
            case 'disconnected': return 3;
            case 'failed': return 4;
            case 'closed': return 5;
            default: return 0;
        }
    }

    _mapIceConnectionState(state) {
        switch (state) {
            case 'new': return 0;
            case 'checking': return 1;
            case 'connected': return 2;
            case 'completed': return 3;
            case 'disconnected': return 4;
            case 'failed': return 5;
            case 'closed': return 6;
            default: return 0;
        }
    }

    _mapIceGatheringState(state) {
        switch (state) {
            case 'new': return 0;
            case 'gathering': return 1;
            case 'complete': return 2;
            default: return 0;
        }
    }

    _mapSignalingState(state) {
        switch (state) {
            case 'stable': return 0;
            case 'have-local-offer': return 1;
            case 'have-remote-offer': return 2;
            case 'have-local-pranswer': return 3;
            case 'have-remote-pranswer': return 4;
            case 'closed': return 5;
            default: return 0;
        }
    }

    /**
     * Clean up all connections
     */
    destroy() {
        for (const [, entry] of this._peerConnections) {
            entry.pc.close();
        }
        this._peerConnections.clear();
        this._dataChannels.clear();
    }
}
