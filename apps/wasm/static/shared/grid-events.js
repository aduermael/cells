...
export class GridEventHandler {
    constructor(canvas, options = {}) {
        ...
        this.onResizeColumn = options.onResizeColumn || (() => {});
        this.onMoveColumn = options.onMoveColumn || (() => {});
        this.onMoveRow = options.onMoveRow || (() => {});
    ...
    _handleWheel(e) {
        ...
        const maxScrollX = Math.max(0, this.sheetInfo.colCount * DEFAULT_COL_WIDTH - this.canvas.clientWidth + HEADER_WIDTH);
        const maxScrollY = Math.max(0, this.sheetInfo.rowCount * DEFAULT_ROW_HEIGHT - this.canvas.clientHeight + HEADER_HEIGHT);
    ...
    _handleMouseDown(e) {
        ...
        if (y < HEADER_HEIGHT && y > 0 && x > HEADER_WIDTH) {
            ...
            if (resizeCol >= 0) {
                ...
                this.resizeStartWidth = this.colWidths.get(resizeCol) || DEFAULT_COL_WIDTH;
    ...
    async _handleMouseUp(e) {
        if (this.isResizing) {
            ...
            if (this.dragSourceIndex !== this.dragTargetIndex && this.dragSourceIndex !== this.dragTargetIndex - 1) {
                const colId = this.getColumnId(this.dragSourceIndex);
                if (colId) {
                    const newSelectedCol = this.dragTargetIndex > this.dragSourceIndex
                        ? this.dragTargetIndex - 1
            ...
            if (this.dragSourceIndex !== this.dragTargetIndex && this.dragSourceIndex !== this.dragTargetIndex - 1) {
                ...
                if (rowId) {
                    const newSelectedRow = this.dragTargetIndex > this.dragSourceIndex
                        ? this.dragTargetIndex - 1
                        : this.dragTargetIndex;
