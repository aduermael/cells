...
export class GridRenderer {
  ...
  render(): void {
    ...
    if (range) {
      ...
      if (this.hasRangeSelection() && this.selectionStart) {
        ...
        const anchorW =
          this.colWidths.get(this.selectionStart.col) || DEFAULT_COL_WIDTH;
        const anchorH =
          this.rowHeights.get(this.selectionStart.row) || DEFAULT_ROW_HEIGHT;
  ...
  drawDragGhost(): void {
    ...
    if (this.isDraggingColumn) {
      ...
      ctx.textBaseline = "middle";
      ctx.fillText(
        this.getColumnHeaderText(this.dragSourceIndex),
        ghostX + colW / 2,
        HEADER_HEIGHT / 2
      );
