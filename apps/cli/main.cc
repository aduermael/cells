...
void print_usage(const char* program_name) {
    std::cerr
        ...
        << "                      .js/.mjs = Office.js (QuickJS); .luau/.lua = Luau\n"
        ...
        << "\n"
        << "Scripting:\n"
        << "  Luau (default) uses the Cells API: getCell, setCell, getSheet, addSheet,\n"
        << "    selectSheet, setFormat, setStyle, print, ...\n"
        << "  JavaScript (.js or Excel.run / Office.onReady) uses the Office.js Excel add-in\n"
        << "    API hosted in QuickJS: Office, Excel, OfficeExtension, Excel.run, context.sync.\n"
        << "  Full API reference: skill SCRIPTING.md (agent skill package) or project docs.\n"
        << "  Examples: setCell(\"A1\", 100)  |  Excel.run(async (ctx) => { ... })\n"
        << "\n"
        ...
        << "  cells out.xlsx -e 'await Excel.run(async (c) => { c.workbook.worksheets.getActiveWorksheet().getRange(\"A1\").values=[[42]]; await c.sync(); })'\n"
        << "  cells -i data.xlsx output.csv --script transform.luau\n"
        << "  cells -i data.xlsx output.xlsx --script transform.js\n"
