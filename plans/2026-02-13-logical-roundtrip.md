...
- [ ] 3a: Implement `SWITCH(expression, value1, result1, ..., [default])` — evaluates expression, compares against each value, returns matching result. With default (odd remaining args after expression), returns default when no match; without default, returns #N/A. If expression is error, propagate it.
