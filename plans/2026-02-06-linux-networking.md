...
- [x] 3c: Alpine/musl full CLI support - **Now working.** Added a system Perl toolchain (`toolchains/perl/`) that wraps `/usr/bin/perl` for OpenSSL assembly generation, bypassing the glibc-linked `relocatable-perl` binaries. Also use preinstalled cmake/make/ninja/pkg-config via `--extra_toolchains` to avoid `rules_foreign_cc` bootstrapping issues on musl/GCC 15. Alpine now builds the full `cells` binary with networking/sync support.
