...
- [x] 4a: Update `tools/serve/main.go` default `-dir` to `dist/wasm`
- [x] 4b: Update `.gitignore` - remove `apps/wasm/dist/`, keep `dist/`
- [x] 4c: Delete `scripts/` directory contents that are now in `tools/` (check.sh, format.sh, lint.sh)
- [x] 4d: Move remaining `scripts/` files to `tools/` (generate_large_cells.py, generate-*.sh, loc-tracker.sh)
- [x] 4e: Delete `scripts/` directory after moving all files
- [x] 4f: Move `examples/` scripts to `tools/examples/` or delete if obsolete
