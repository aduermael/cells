...
PendingOperation::PendingOperation(Operation op, ID peerId)
    : op(std::move(op)), sourcePeerId(peerId) {}

// ============================================================================
// Workbook - Pending Operations
// ============================================================================

void Workbook::addPendingOp(const Operation& op) {
    // Only CELL_SET_VALUE can be pending
    if (op.type != OpType::CELL_SET_VALUE) {
        return;
    }

    // Replace existing pending op for the same target (debounce)
    for (auto& pending : _pendingOps) {
        if (pending.op.target_id == op.target_id && pending.sourcePeerId.isNull()) {
            pending.op = op;
            return;
        }
    }

    // Add new pending op (local, so empty peer ID)
    _pendingOps.emplace_back(op, ID());
}

void Workbook::addRemotePendingOp(const Operation& op, const ID& peerId) {
    // Only CELL_SET_VALUE can be pending
    if (op.type != OpType::CELL_SET_VALUE) {
        return;
    }

    // Replace existing pending op from the same peer for the same target
    for (auto& pending : _pendingOps) {
        if (pending.op.target_id == op.target_id && pending.sourcePeerId == peerId) {
            pending.op = op;
            return;
        }
    }

    // Add new pending op from this peer
    _pendingOps.emplace_back(op, peerId);
}
...
std::vector<Operation> Workbook::commitPendingOps() {
    std::vector<Operation> committed;

    // Commit all local pending operations
    for (auto it = _pendingOps.begin(); it != _pendingOps.end();) {
        if (it->sourcePeerId.isNull()) {
            // Apply to oplog
            _oplog->addOperation(it->op);
            committed.push_back(it->op);
...
std::vector<Operation> Workbook::commitPendingOpsForTarget(const ID& targetId) {
    std::vector<Operation> committed;

    // Find and commit local pending ops for this target
    for (auto it = _pendingOps.begin(); it != _pendingOps.end();) {
        if (it->op.target_id == targetId && it->sourcePeerId.isNull()) {
            // Apply to oplog
            _oplog->addOperation(it->op);
            committed.push_back(it->op);
            it = _pendingOps.erase(it);
...
const std::vector<PendingOperation>& Workbook::getPendingOps() const {
    return _pendingOps;
}

const Operation* Workbook::getPendingOpForTarget(const ID& targetId) const {
    // Return the most recent pending op for this target
    // Priority: local > remote (but we just return the first match for now)
    // In practice, there should be at most one local and one remote pending per target

    // First, look for local pending op
    for (const auto& pending : _pendingOps) {
        if (pending.op.target_id == targetId && pending.sourcePeerId.isNull()) {
            return &pending.op;
        }
    }

    // Then, look for remote pending op
    for (const auto& pending : _pendingOps) {
        if (pending.op.target_id == targetId) {
            return &pending.op;
        }
    }
...
bool Workbook::hasPendingOps() const {
    return !_pendingOps.empty();
}

size_t Workbook::pendingOpsCount() const {
