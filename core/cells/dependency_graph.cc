...
// Helper to extract cross-sheet references from an AST (recursive)
void extractCrossSheetRefsHelper(const ASTNode* node, std::vector<CrossSheetRef>& refs) {
    if (!node) {
        return;
    }
    ...
    switch (node->type) {
        case ASTNodeType::CELL_REF: {
            auto* cellRef = static_cast<const CellRefNode*>(node);
            // Only track if sheetId is set (cross-sheet reference)
            if (!cellRef->sheetId.empty() && !cellRef->cellId.empty()) {
                CrossSheetRef ref;
                ref.sheetId = ID(cellRef->sheetId);
                ref.cellId = ID(cellRef->cellId);
                ref.type = CrossSheetRef::Type::CELL;
                refs.push_back(ref);
            }
            break;
        }
        ...
        case ASTNodeType::RANGE_REF: {
            auto* rangeRef = static_cast<const RangeRefNode*>(node);
            if (rangeRef->topLeft && rangeRef->bottomRight) {
                // Check if this is a cross-sheet range (topLeft has sheetId)
                if (!rangeRef->topLeft->sheetId.empty() && !rangeRef->topLeft->cellId.empty() &&
                    !rangeRef->bottomRight->cellId.empty()) {
                    CrossSheetRef ref;
                    ref.sheetId = ID(rangeRef->topLeft->sheetId);
                    ref.startCellId = ID(rangeRef->topLeft->cellId);
                    ref.endCellId = ID(rangeRef->bottomRight->cellId);
                    ref.type = CrossSheetRef::Type::RANGE;
                    refs.push_back(ref);
                }
            }
            break;
        ...
        case ASTNodeType::FUNCTION_CALL: {
            auto* funcCall = static_cast<const FunctionCallNode*>(node);
            for (const auto& arg : funcCall->args) {
                extractCrossSheetRefsHelper(arg.get(), refs);
            }
            break;
        }
        ...
        case ASTNodeType::BINARY_OP: {
            auto* binOp = static_cast<const BinaryOpNode*>(node);
            extractCrossSheetRefsHelper(binOp->left.get(), refs);
            extractCrossSheetRefsHelper(binOp->right.get(), refs);
            break;
        }

        case ASTNodeType::UNARY_OP: {
            auto* unaryOp = static_cast<const UnaryOpNode*>(node);
            extractCrossSheetRefsHelper(unaryOp->operand.get(), refs);
            break;
        }
        ...
        case ASTNodeType::ERROR_NODE: {
            auto* errorNode = static_cast<const ErrorNode*>(node);
            for (const auto& child : errorNode->partialChildren) {
                extractCrossSheetRefsHelper(child.get(), refs);
            }
            break;
        }
...
std::vector<CrossSheetRef> extractCrossSheetRefs(const ASTNode* ast) {
    std::vector<CrossSheetRef> refs;
    extractCrossSheetRefsHelper(ast, refs);
    return refs;
}
