...
std::string RefConverter::adjustFormulaReferences(const std::string& formula, int colOffset,
                                                  int rowOffset) {
    if (formula.empty()) {
        return formula;
    }
        ...
        if (canStartRef && isA1RefStart(formula, i)) {
            ...
            if (len > 0 && ref.valid) {
                ...
                if (i + len < formula.size() && formula[i + len] == ':') {
                    ...
                    const std::string startRef = adjustSingleRef(ref, colOffset, rowOffset);
                    ...
                    if (endLen > 0 && endRef.valid) {
                        const std::string endRefStr = adjustSingleRef(endRef, colOffset, rowOffset);
                        result += startRef;
                        result += ':';
                        result += endRefStr;
                        i += len + 1 + endLen;
                ...
                const std::string adjustedRef = adjustSingleRef(ref, colOffset, rowOffset);
                result += adjustedRef;
                i += len;
                continue;
            }
...
std::string RefConverter::adjustSingleRef(const CellRef& ref, int colOffset, int rowOffset) {
    // Start with the original ref type
    CellRef adjusted = ref;

    // Adjust column if relative
    if (!ref.isAbsoluteCol()) {
        const int newCol = static_cast<int>(ref.colIndex) + colOffset;
        if (newCol < 0) {
            return "#REF!";
        }
        adjusted.colIndex = static_cast<size_t>(newCol);
    }

    // Adjust row if relative
    if (!ref.isAbsoluteRow()) {
        const int newRow = static_cast<int>(ref.rowIndex) + rowOffset;
        if (newRow < 0) {
            return "#REF!";
        }
        adjusted.rowIndex = static_cast<size_t>(newRow);
    }
