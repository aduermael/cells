...
struct ID {
    ...
    bool operator==(const ID& other) const {
        return std::memcmp(data, other.data, ID_LENGTH) == 0;
    }
...
inline CellValueType charToValueType(char c) {
    switch (c) {
        ...
        case 'f': return CellValueType::FORMULA;
        case 'b': return CellValueType::BOOLEAN;
        case 'e': return CellValueType::ERROR;
        case 'd': return CellValueType::DATE;
        case 't': return CellValueType::DATE_TIME;
        default:  return CellValueType::STRING;  // Default to string
    }
}
...
inline char valueTypeToChar(CellValueType type) {
    switch (type) {
        ...
        case CellValueType::STRING:    return 's';
        case CellValueType::FORMULA:   return 'f';
        case CellValueType::BOOLEAN:   return 'b';
        case CellValueType::ERROR:     return 'e';
        case CellValueType::DATE:      return 'd';
        case CellValueType::DATE_TIME: return 't';
    }
...
inline CellError stringToError(const std::string& s) {
    if (s == ERROR_VALUE_STR) return CellError::VALUE;
    if (s == ERROR_REF_STR) return CellError::REF;
    if (s == ERROR_NAME_STR) return CellError::NAME;
    if (s == ERROR_DIV_STR) return CellError::DIV;
    if (s == ERROR_NULL_STR) return CellError::NULL_REF;
    if (s == ERROR_NUM_STR) return CellError::NUM;
    if (s == ERROR_CIRCULAR_STR) return CellError::CIRCULAR;
...
inline const char* errorToString(CellError error) {
    switch (error) {
        case CellError::NONE:     return "";
        case CellError::VALUE:    return ERROR_VALUE_STR;
        case CellError::REF:      return ERROR_REF_STR;
        case CellError::NAME:     return ERROR_NAME_STR;
        case CellError::DIV:      return ERROR_DIV_STR;
        case CellError::NULL_REF: return ERROR_NULL_STR;
        case CellError::NUM:      return ERROR_NUM_STR;
        case CellError::CIRCULAR: return ERROR_CIRCULAR_STR;
    }
    return "";
}
