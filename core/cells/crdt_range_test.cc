...
TEST_F(CRDTRangeTest, AddRangeCreatesRange) {
    ...
    // Build payload
    std::string payload = "{\"sheet\":\"" + sheetId.toString() + "\",";
    payload += "\"startCol\":\"" + colIds[0].toString() + "\",";
...
TEST_F(CRDTRangeTest, AddRangeDuplicateIdReturnsAlreadyApplied) {
    ...
    payload += "\"startCol\":\"" + colIds[0].toString() + "\",";
    payload += "\"startRow\":\"" + rowIds[0].toString() + "\",";
    payload += "\"endCol\":\"" + colIds[1].toString() + "\",";
    payload += "\"endRow\":\"" + rowIds[1].toString() + "\",";
    payload += "\"flags\":0}";

    // First add should succeed
    Operation op1 = makeRangeSetOp(*workbook, rangeId, payload);
    ApplyResult result1 = applyOperation(*workbook, op1);
    EXPECT_EQ(result1, ApplyResult::SUCCESS);

    // Second SET with same range ID is an update (no-op if nothing changed)
    // With unified SET operations, this is just SUCCESS (not ALREADY_APPLIED)
    Operation op2 = makeRangeSetOp(*workbook, rangeId, payload);
    ApplyResult result2 = applyOperation(*workbook, op2);
    EXPECT_EQ(result2, ApplyResult::SUCCESS);
}
...
TEST_F(CRDTRangeTest, AddRangeInvalidColumnReturnsInvalidTarget) {
    ...
    std::string payload = "{\"startCol\":\"" + fakeColId.toString() + "\",";
    payload += "\"startRow\":\"" + rowIds[0].toString() + "\",";
    payload += "\"endCol\":\"" + colIds[1].toString() + "\",";
    payload += "\"endRow\":\"" + rowIds[1].toString() + "\",";
    payload += "\"flags\":0}";
...
TEST_F(CRDTRangeTest, RemoveRangeDeletesRange) {
    ...
    // First add a range
    std::string addPayload = "{\"sheet\":\"" + sheetId.toString() + "\",";
    addPayload += "\"startCol\":\"" + colIds[0].toString() + "\",";
    addPayload += "\"startRow\":\"" + rowIds[0].toString() + "\",";
    addPayload += "\"endCol\":\"" + colIds[1].toString() + "\",";
    addPayload += "\"endRow\":\"" + rowIds[1].toString() + "\",";
    addPayload += "\"flags\":0}";

    Operation addOp = makeRangeSetOp(*workbook, rangeId, addPayload);
    ...
    // Now remove it
    std::string removePayload = "{\"sheet\":\"" + sheetId.toString() + "\"}";
    Operation removeOp = makeRangeDeleteOp(*workbook, rangeId, removePayload);
...
TEST_F(CRDTRangeTest, RemoveRangeIdempotent) {
    ...
    std::string payload = "{\"sheet\":\"" + sheetId.toString() + "\"}";
    Operation op = makeRangeDeleteOp(*workbook, rangeId, payload);
    ApplyResult result = applyOperation(*workbook, op);
...
TEST_F(CRDTRangeTest, UpdateCornersResizesRange) {
    ...
    std::string updatePayload = "{\"sheet\":\"" + sheetId.toString() + "\",";
    updatePayload += "\"startCol\":\"" + colIds[0].toString() + "\",";
    updatePayload += "\"startRow\":\"" + rowIds[0].toString() + "\",";
    updatePayload += "\"endCol\":\"" + colIds[3].toString() + "\",";
    updatePayload += "\"endRow\":\"" + rowIds[3].toString() + "\"}";

    Operation updateOp = makeRangeSetOp(*workbook, rangeId, updatePayload);
...
TEST_F(CRDTRangeTest, SetCornersCreatesNewRange) {
    const ID rangeId = generate_id();

    // SET on a non-existent range creates it
    std::string payload = "{\"sheet\":\"" + sheetId.toString() + "\",";
    payload += "\"startCol\":\"" + colIds[0].toString() + "\",";
    payload += "\"startRow\":\"" + rowIds[0].toString() + "\",";
    payload += "\"endCol\":\"" + colIds[2].toString() + "\",";
    ...
    // Unified SET creates the range if it doesn't exist
    EXPECT_EQ(result, ApplyResult::SUCCESS);
...
TEST_F(CRDTRangeTest, SetFlagsOnNonExistentRangeWithoutCornersReturnsInvalidPayload) {
    const ID rangeId = generate_id();

    // Try to create/update a range with only flags, no corners
    std::string payload = "{\"sheet\":\"" + sheetId.toString() + "\",";
    payload += "\"flags\":1}";
    ...
    // Without corners, can't create a new range - INVALID_PAYLOAD
...
TEST_F(CRDTRangeTest, MultipleOverlappingRangesAtSamePosition) {
    ...
    std::string payload1 = "{\"sheet\":\"" + sheetId.toString() + "\",";
    payload1 += "\"startCol\":\"" + colIds[0].toString() + "\",";
    payload1 += "\"startRow\":\"" + rowIds[0].toString() + "\",";
    payload1 += "\"endCol\":\"" + colIds[2].toString() + "\",";
    payload1 += "\"endRow\":\"" + rowIds[2].toString() + "\",";
    payload1 += "\"flags\":1}";

    // Range 2: B2:D4 with STYLE flag
    std::string payload2 = "{\"sheet\":\"" + sheetId.toString() + "\",";
    payload2 += "\"startCol\":\"" + colIds[1].toString() + "\",";
    payload2 += "\"startRow\":\"" + rowIds[1].toString() + "\",";
    payload2 += "\"endCol\":\"" + colIds[3].toString() + "\",";
    payload2 += "\"endRow\":\"" + rowIds[3].toString() + "\",";
    payload2 += "\"flags\":2}";

    Operation op1 = makeRangeSetOp(*workbook, range1Id, payload1);
    Operation op2 = makeRangeSetOp(*workbook, range2Id, payload2);
...
TEST_F(CRDTRangeTest, SetStyleWithStyleBufferBase64Payload) {
    ...
    // Use makeRangeSetStyleOp with StyleBuffer
    Operation styleOp = makeRangeSetStyleOp(*workbook, rangeId, style);
    ApplyResult result = applyOperation(*workbook, styleOp);
...
TEST_F(CRDTRangeTest, InvalidBase64ReturnsInvalidPayload) {
    ...
    // Try to set with invalid base64 (using makeRangeSetOp to test parser directly)
    std::string invalidPayload = "{\"sty\":\"not-valid-base64!!!\"}";
    Operation styleOp = makeRangeSetOp(*workbook, rangeId, invalidPayload);
    ApplyResult result = applyOperation(*workbook, styleOp);

    // Operation should succeed but invalid style is ignored
    EXPECT_EQ(result, ApplyResult::SUCCESS);
}
