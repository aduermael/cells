...
# Collab + verbose signaling (full SDP/ICE) for local CLI↔browser debug.
cd tools/serve && go run . -port 8081 -dir "$REPO_ROOT/dist/wasm" \
  -enable-collab -signaling-verbose
